...
io.on("connection", (socket) => {
    ...
    socket.on("invite", (payload) => {
        ...
        if ((typeof username == "undefined") || (username === null) || (username === "")) {
            response = {
                result: 'fail',
                message: 'the user that was invited does not have a room registered'
            }
            socket.emit("invite_response", response);
            serverLog("invite command failed", JSON.stringify(response));
            return;
        ...
        /* Make sure that the invited player is client was put in the room */
    ...
    socket.on("uninvite", (payload) => {
        ...
        if ((typeof username == "undefined") || (username === null) || (username === "")) {
            response = {
                result: 'fail',
                message: 'the user that was uninvited does not have a room registered'
            }
            socket.emit("uninvited", response);
            serverLog("uninvite command failed", JSON.stringify(response));
            return;
    ...
    socket.on("game_start", (payload) => {
        ...
        if ((typeof username == "undefined") || (username === null) || (username === "")) {
            response = {
                result: 'fail',
                message: 'the user that was engaged to play does not have a room registered'
            }
            socket.emit("game_start_response", response);
            serverLog("game_start command failed", JSON.stringify(response));
            return;
    ...
    /* send_chat_message command handller */
    /* expected payload:
        {
    ...
    */
    socket.on("send_chat_message", (payload) => {
        serverLog(
            'Server received a command', '\'send_chat_message\'', JSON.stringify(payload));
        /* check the data coming from the client is good */
